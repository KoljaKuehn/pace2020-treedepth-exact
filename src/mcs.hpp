...
std::vector<int> Mcs(const FGraph& graph);
void LbTriang(FGraph& graph, std::mt19937& gen, int vari, int upd_cnt);
int Treewidth(const FGraph& graph);
std::vector<FBitset> ChordalMinseps(const FGraph& graph);
...
inline std::vector<int> Mcs(const FGraph& graph) {
  ...
  for (int it = graph.n() - 1; it >= 0; it--) {
    if (labels[max_label].size() == 0) { max_label--; it++; continue; }
...
inline int HeurLbt(const FGraph& graph, int v) {
  int fec = 0;
  for (const FBitset& cn : graph.CompNeighsBit(graph.adj_mat2_[v])) {
    fec += graph.FillSize(cn);
  }
  return fec;
}

inline void LbTriang(FGraph& graph, std::mt19937& gen, int vari, int upd_cnt) {
  Timer lbt;
  lbt.start();
  ...
  for (int it = 0; it < graph.n(); it++) {
    ...
    for (int i = 0; i < graph.n(); i++) {
      if (upd[i]) {
        for (int nb : graph.Neighbors(i)) score[nb]++;
        upd[i] = 0;
        score[i] += 2;
      }
    }
    ...
    for (int i = 0; i < graph.n(); i++) {
      if (hs[i] >= 0 && score[i] > 0) to_upd.push_back({score[i], i});
    }
...
inline int Treewidth(const FGraph& graph) {
...
inline std::vector<FBitset> ChordalMinseps(const FGraph& graph) {
  if (graph.m() == 0) return {};
  std::vector<int> order = Mcs(graph);
  std::vector<int> inv_order = utils::PermInverse(order);
  ...
  for (int i = graph.n()-2; i >= 0; i--) {
    ...
    for (int nx : graph.Neighbors(order[i])) {
      if (inv_order[nx] > i) cnt++;
    }
    if (cnt <= prev && cnt > 0) {
      FBitset ms;
      for (int nx : graph.Neighbors(order[i])) {
        if (inv_order[nx] > i) ms.SetTrue(nx);
      }
      ret.push_back(ms);
    }
    prev = cnt;
