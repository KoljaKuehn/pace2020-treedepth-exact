...
MSSolve::MSSolve(const FGraph& graph) : graph_(graph) {
...
int MSSolve::PieceId(const FBitset& piece, bool insert, bool expect) {
  assert(pcs_.size() >= 1);
  if (insert) {
...
int MSSolve::MsPieceId(const FBitset& piece, bool insert, bool expect) {
...
bool MSSolve::Isom(const FBitset& v1, const FBitset& v2) const {
...
bool MSSolve::Go(FBitset vert, int k, const vector<Edge>& parent_edges,
