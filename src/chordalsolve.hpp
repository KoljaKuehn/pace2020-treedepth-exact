...
struct MinsepInfo {
  ...
  bool operator==(const MinsepInfo& other) const { return minsep == other.minsep; }
  int ExpSize() const { return comp_size; }
};

struct PieceC {
  PieceC() { lb = -1; ub = (int)1e9; }
  int lb, ub;
  std::vector<MinsepInfo> minseps;
};

class ChordalSolve {
 public:
  ChordalSolve(const FGraph& graph);
 ...
 private:
  ...
  std::vector<PieceC> pcs_;
  FBitsetMap bs_cac_;
  FLBSieve lb_sieve_;
  bool Go(FBitset vert, int k, const std::vector<Edge>& parent_edges,
          const std::vector<MinsepInfo>& parent_minseps, int parent_n);
  bool Reco(FBitset vert, int k, const std::vector<Edge>& parent_edges);
};

inline ChordalSolve::ChordalSolve(const FGraph& graph) : graph_(graph) {
  assert(!bs_cac_.Inited());
  bs_cac_ = FBitsetMap(2, 2);
  assert(bs_cac_.Inited());
  pcs_.push_back({});
  lb_sieve_ = FLBSieve(graph.n(), graph.n()+1);
}

inline int ChordalSolve::PieceId(const FBitset& piece, bool insert, bool expect) {
  assert(pcs_.size() >= 1);
  if (insert) {
...
inline bool ChordalSolve::Reco(FBitset vert, int k, const vector<Edge>& parent_edges) {
  ...
  for (const MinsepInfo& ms : pcs_[pc_id].minseps) {
    ...
    for (const FBitset& bc : bcs) {
      ...
      if (pcs_[bcid].ub > k - ms.sep_size) { ok = false; break; }
    }
    if (ok) {
      for (const FBitset& bc : bcs) assert(Reco(bc, k - ms.sep_size, tg_edges));
      for (int v : ms.minsep) { assert(k >= 1); resu_[v] = k--; }
      return true;
    }
  }
  assert(false);
...
inline bool ChordalSolve::Go(FBitset vert, int k, const vector<Edge>& parent_edges,
                              const vector<MinsepInfo>& parent_minseps, int parent_n) {
  ctott.start();
  ...
  if ((int64_t)t_graph.m() == (int64_t)n*(n-1)/2) {
    pcs_[pc_id].ub = n; pcs_[pc_id].lb = n;
    csubt.start(); lb_sieve_.Insert(vert, pcs_[pc_id].lb); csubt.stop();
    return false;
  } else {
  ...
  if (pcs_[pc_id].minseps.empty()) {
    if (glo_k == k && n > parent_n/2 && !parent_minseps.empty()) {
      ...
      for (const auto& pms : parent_minseps) {
        ...
        FBitset vert2 = vert; vert2.TurnOff(pms.minsep);
        FBitset vert4 = pms.minsep; vert4 &= vert;
      ...
      cmst2.start();
      for (const FBitset& ms : mcs::ChordalMinseps(t_graph)) {
        int sep_size = ms.Popcount();
        assert(sep_size < k && sep_size >= 1);
        FBitset vert2 = vert; vert2.TurnOff(ms);
        int fcs = 0, comp_size = 0, sz = n - sep_size;
        for (int i = 0; i < t_graph.n(); i++) {
          if (vert2.Get(i)) {
            FBitset vert3 = t_graph.adj_mat2_[i];
    ...
    cmst3.start();
    auto cmp = [&](const MinsepInfo& a, const MinsepInfo& b) {
      if (a.comp_size != b.comp_size) return a.comp_size < b.comp_size;
      if (a.sep_size  != b.sep_size)  return a.sep_size  < b.sep_size;
      return a.minsep < b.minsep;
    };
    std::sort(pcs_[pc_id].minseps.begin(), pcs_[pc_id].minseps.end(), cmp);
    pcs_[pc_id].minseps.erase(
        std::unique(pcs_[pc_id].minseps.begin(), pcs_[pc_id].minseps.end()),
        pcs_[pc_id].minseps.end());
    cmst3.stop();
  }
  int branches = 0;
  for (const MinsepInfo& ms : pcs_[pc_id].minseps) {
    if (branches >= 1 && timer_.get() > time_limit_) break;
    if (ms.comp_size >= n/2 + k - ms.sep_size + tune_) continue;
    ...
    for (const FBitset& bc : bcs) {
      ...
      if (pcs_[bcid].lb > k - ms.sep_size) { ok = false; break; }
    }
    int tans = 0;
    if (ok) {
      for (const FBitset& bc : bcs) {
        if (!Go(bc, k - ms.sep_size, tg_edges, pcs_[pc_id].minseps, n)) { ok = false; break; }
        else tans = max(tans, ms.sep_size + pcs_[PieceId(bc, false, true)].ub);
      }
    }
    vert |= ms.minsep;
    if (ok) {
...
inline int ChordalSolve::Solve(int goal, int tune, double timelimit) {
  ctott.stop();
  csubt.clear(); cmst1.clear(); cmst2.clear(); cmst3.clear(); ctott.clear(); ctg.clear();
  ...
  for (int k = goal; k >= 0; k--) {
    ...
    if (!Go(vert, k, graph_edges, {}, graph_.n())) {
      ...
      tune_++;
      lb_sieve_ = FLBSieve(graph_.n(), graph_.n()+1);
      for (int i = 1; i < (int)pcs_.size(); i++) pcs_[i].lb = 0;
      k++;
    }
  }
  return 0;
...
inline vector<int> ChordalSolve::Get(int goal) {
